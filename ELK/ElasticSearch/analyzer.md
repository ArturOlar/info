...
- Еластік надає великий список вдбудованих `tokenizers`, таких як: standard (default), whitespace, keyword, letter та інші.
...
Роглянемо приклад: якщо спробуємо проаналізувати цей текст за допомогою `standard` аналізатора, то побачимо дивний результат, а саме що html-теги не були видалені. Це тому що, в `standard` аналізаторі немає `char filter` для видалення html-тегів.
```
POST /_analyze
{
  "analyzer": "standard",
